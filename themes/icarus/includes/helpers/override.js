...
module.exports = function(hexo) {
  hexo.extend.helper.register("_list_archives", function() {
    ...
    const $ = cheerio.load(this.list_archives({ format: "YYYY/MM" }), {
      decodeEntities: false
    });
    $(".archive-list-item").each(function() {
      __archives.push({
        url: $(this)
          .find(".archive-list-link")
          .attr("href"),
        name: $(this)
          .find(".archive-list-link")
          .text(),
        count: $(this)
          .find(".archive-list-count")
          .text()
      });
    });
    return __archives;
  ...
  hexo.extend.helper.register("_list_categories", function() {
    ...
    const $ = cheerio.load(this.list_categories({ depth: 2 }), {
      decodeEntities: false
    });
    function traverse(root) {
      ...
      root.find("> .category-list-item").each(function() {
        const category = {
          url: $(this)
            .find("> .category-list-link")
            .attr("href"),
          name: $(this)
            .find("> .category-list-link")
            .text(),
          count: $(this)
            .find("> .category-list-count")
            .text()
        };
        if ($(this).find("> .category-list-child").length) {
          category["children"] = traverse(
            $(this).find("> .category-list-child")
          );
        }
  ...
  hexo.extend.helper.register("_js", function(
    url,
    defer = false,
    async = false
  ) {
    ...
    return `<script src="${urlFor(url)}"${async ? " async" : ""}${
      defer ? " defer" : ""
    }></script>`;
  });
  ...
  hexo.extend.helper.register("_partial", function(name, locals, options = {}) {
    ...
    let { path } =
      hexo.theme.getView(join(dirname(currentView), name)) ||
      hexo.theme.getView(name);
    path = join(
      view_dir,
      path.substring(0, path.length - extname(path).length) + ".locals.js"
    );
    ...
    const fragment = relative(
      view_dir,
      path.substring(0, path.length - ".locals.js".length)
    );
    ...
    if (!__fragmentCache.has(cacheId)) {
      __fragmentCache.set(
        cacheId,
        partial(name, _locals, { cache: false, only: options.only || false })
      );
    }
